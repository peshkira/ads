...
public class MembershipMessageListener implements AdvancedMessageListener {
    ...
    private void refreshProxyData(MembershipInfo info) {
        if (info.getJoined() != null) {
    ...
    private void synchronizeState(MembershipInfo info) {
        ...
        for (SpreadGroup g : members) {
            if (!g.toString().startsWith(server.getServerId(), 1))
                server.askForServerReference(g);
        }
    }
    ...
    private void askForState(int memberIndex) {
        ...
        try {
            SpreadMessage request = ServerMessageFactory.getInstance().createSafeMessage(
                    ServerConstants.MSG_GET_SERVER_STATE, null, server.getOwnGroup(),
                    server.getGroupMembers().get(memberIndex));
    ...
    private boolean isSelfJoinMessage(MembershipInfo info) {
        if (info.isCausedByJoin() && info.getGroup().equals(server.getServerGroup())
                && info.getJoined().toString().startsWith(server.getServerId(), 1))
            return true;
        return false;
    }
}
